...
def sig_calibration(flag,camera_):
    result = list()
    source_dir_path = os.path.dirname(os.path.realpath(__file__)).replace("cameraCalibration","static\\res_pictures\\"+flag+"_calibration_"+camera_+"\\")
    starttime = time.time()
    cbrow = 9
    cbcol = 13    #纵横角点
    ...
    compute_distance_path = os.path.dirname(os.path.realpath(__file__)).replace("cameraCalibration","static\\res_pictures\\result\\calibration\\"+flag+"_"+camera_+"calibresult.png")
...
def stereo_Calibration(flag):
    ...
    left_camera_path = os.path.dirname(os.path.realpath(__file__)).replace('cameraCalibration','static\\res_pictures\\')
    leftCamera = left_camera_path+flag+"_calibration_left\\"
    #left_camera_path = "/home/monkiki/PycharmProjects/stereo_vision/stereo_vision/checkboard_img_dir/leftCamera_img_dir/"
    right_camera_path = os.path.dirname(os.path.realpath(__file__)).replace('cameraCalibration','static\\res_pictures\\')
    rightCamera = right_camera_path+flag+"_calibration_right\\"
